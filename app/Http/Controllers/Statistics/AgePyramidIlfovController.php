...
class AgePyramidIlfovController extends Controller
{
  ...
  public function index()
  {
    ...
    $ilfov_villages = $ilfov->getVillages('Ilfov');
    $agePyramid->statsUpdate($youngest, $oldest, $ilfov_villages, 'Ilfov');
    //Ilfov youngest and oldest
    ...
    return view('charts/ilfov/age',['menbar'=>$menbar],$data);
  }
  public function age_response()
  {
    ...
    $village = $villages->getVillages('Ilfov');
    ...
    foreach ($graphdatas as $graphdata)
    {
      if ($graphdata->age <= $oldest){
          $sum[] = $graphdata->ilfov_male * -1;
          $fsum[] = $graphdata->ilfov_female;
          }
    }
