...
class StatSkillcatIlfovController extends Controller
{
  ...
  public function open_table_skill(Skill $skill)
  {
    ...
    //Load api reference
    $api=url('/ilfov/skill_cat_data_rom');
    //Assign label
    foreach ($cats as $cat)
      {
    ...
    $skillCat->updateSkillCat($this->SkillCatTraditional, $this->SkillCatCraft, $this->SkillCatMusicians, $this->SkillCatAgriculture, $this->SkillCatService, $this->SkillCatHigherStatus, $this->SkillCatNotSpecified, 'Ilfov');

    $data['skills']=Skillcat::all();
    //End data for table
    return view('charts/ilfov/open_cat_skill',['rom_skills'=>$rom_skills],$data);
  ...
  public function response_skill()
  {
    ...
    {
      $sum[] = $skillcat->ilfov_skillcat_number;
    }
